			...
			if(currChar == EOF){
                        ...
                        if(messageSize == sizeof(lineWrite)-1 || currChar == EOF){
                                ...
                                while(totalBytesSent < messageSize){
                                        bytesSent = send(sock, lineWrite+totalBytesSent, messageSize-totalBytesSent, 0);
                                        if(bytesSent <= 0){
                                                fprintf(stderr, "tcp_server: errorsending response\n");
						close(sock);
                                                return -1;
                                        }
                                        totalBytesSent += bytesSent;
...
main(int argc, char ** argv)
{
	...
	if(connectSock < 0){
        ...
        /* handle connections */
        ret = handle_connection(connectSock);
	if(ret < 0){
    	...
    	//close(connectSock); //doing this in handle connection
    }
    close(acceptSock);
    return 0;
}