...
struct tcp_connection {
    ...
    //keep track of sequence number and acknowledgement numbers expected by the host
    uint32_t conSN;
    uint32_t conAN;

    struct pet_timeout * timeout;
    //booleans used to track progress of retransmission
    int                  timeoutStarted; //indicates whether the countdown has started(timeout added)
    int                  timedOut;       //indicates whether a timeout has expired
    int                  resending;      //indicates whether we retransmit a packet (use con->buf)
    //properties of last sent packet in case need to retransmit
    int      synSet;
    int      ackSet;
    int      finSet;
    int      sendingPayload;
    ...
    //counter is used to test the sending of timeouts
    int counter; //only for testing purposes, counter is not necessary for a working implementation
